...
export type Question = {
  ...
  spaceLines: number;
  grading: { mode: 'exact' | 'partial' | 'manual'; wrongPenalty?: number };
};
export type Exam = {
  schemaVersion: '1.0';
  id: string;
...
export type Attempt = {
  ...
  flagged: string[];
  focusMode: boolean;
  events: { at: number; kind: 'fullscreen_exit' | 'tab_hidden' }[];
};
export type Workspace = { version: 1; exams: Exam[]; attempts: Attempt[] };
...
export function scoringText(q: Question) {
  return !isAuto(q)
    ? '人工 / AI 按评分细则逐项给分'
    : `${q.grading.mode === 'exact' ? '全部选对得满分' : '未选错时按正确选项比例给分'}；错选${q.grading.wrongPenalty ? `扣 ${q.grading.wrongPenalty} 分` : '得 0 分'}；未作答 0 分`;
}
...
export function validateExam(input: unknown): Exam {
  ...
  function unique(values: unknown[], path: string) {
    need(new Set(values).size === values.length, path, 'ID 或内容重复');
  }
  function checkOptions(v: unknown, path: string) {
    ...
    if (Array.isArray(v)) {
      v.forEach((o, i) => {
        if (!obj(o)) {
          need(false, `${path}[${i}]`, '需要对象');
          return;
        }
        id(o.id, `${path}[${i}].id`);
        need(str(o.text), `${path}[${i}].text`, '不能为空');
      });
      unique(
        v.map((o) => o?.id),
        path,
  ...
  if (!obj(input)) throw new Error('根节点必须是 JSON 对象。');
  const e = input;
  need(e.schemaVersion === '1.0', 'schemaVersion', '仅支持 "1.0"');
  id(e.id, 'id');
  need(str(e.title), 'title', '试卷标题不能为空');
  need(str(e.subject), 'subject', '科目不能为空');
  need(str(e.instructions), 'instructions', '考试说明不能为空');
  need(
  ...
  need(num(e.totalPoints) && e.totalPoints > 0, 'totalPoints', '应为正数');
  if (e.description !== undefined)
    need(typeof e.description === 'string', 'description', '应为文本');
  const materials = Array.isArray(e.materials) ? e.materials : [];
  if (e.materials !== undefined)
    need(Array.isArray(e.materials), 'materials', '应为数组');
  materials.forEach((m: any, i: number) => {
    if (!obj(m)) {
      need(false, `materials[${i}]`, '需要对象');
      return;
    }
  ...
  if (Array.isArray(e.sections)) {
    ...
    e.sections.forEach((s: any, si: number) => {
      ...
      id(s.id, `${sp}.id`);
      need(str(s.title), `${sp}.title`, '不能为空');
      ...
      s.questions.forEach((q: any, qi: number) => {
        ...
        need(str(q.prompt), `${p}.prompt`, '题干不能为空');
        need(
          num(q.points) &&
            q.points > 0 &&
        ...
        need(obj(q.grading), `${p}.grading`, '必须指定评分模式');
        if (
          ['single_choice', 'multiple_choice', 'true_false'].includes(q.type)
        ) {
          ...
          if (q.type === 'fill_blank') {
            ...
            if (Array.isArray(q.blanks)) {
              ...
              q.blanks.forEach((b: any, i: number) => {
                if (!obj(b)) {
                  need(false, `${p}.blanks[${i}]`, '需要对象');
                  return;
                }
                id(b.id, `${p}.blanks[${i}].id`);
                need(str(b.label), `${p}.blanks[${i}].label`, '不能为空');
                need(
