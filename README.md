...
## 验证
```sh
node --experimental-strip-types --test tests/exam.test.mjs tests/editor.test.mjs
...
默认英文，右上角切换简体中文，偏好保存在当前浏览器。已有试卷、历史作答和答案内容不做翻译或迁移。内置示例和完整规范有中英文版本。新增 Exam prompt 页面可设置题量、时间、试卷语言和重点，并附加规范复制/下载。

所有填空标签、选项、匹配项目和表格标题经过统一数学渲染；打印文件同样支持。底栏固定三列布局，保存状态显示延迟消除短暂跳动；题号导航单独 memo，输入不改变完成数量时不重新渲染。

新增验证：`node tests/localization.test.mjs`。使用无浏览器 React renderer 验证连续无关更新不重渲染题号，检查标签公式、英文默认、语言切换、打印语言和 Prompt 比例与约束。
